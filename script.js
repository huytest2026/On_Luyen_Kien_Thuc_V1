...
(function(){
  ...
  window.initToeicReadingBank=function(){jsonp('toeicreadinginit',{maHS:student()||'Bảo'}).then(r=>status(r.ok?'✅ Sheet '+r.sheet+' đã sẵn sàng. Có '+r.count+' câu.':'❌ '+(r.message||'Không thực hiện được.'))).catch(e=>status('❌ '+e.message))};
