/* V45.1.7 — Smart Adaptive Parser + Validator + explicit Part 6 boundary repair + robust loader. */
/* V45.1.5 — Adaptive validation, test/part continuity checks, Part 6 group repair. */
...
let pdfjsLib = null;
function getPdfjs(){
  pdfjsLib = window.pdfjsLib || globalThis.pdfjsLib || pdfjsLib;
  return pdfjsLib;
}
...
function adaptiveConfidence(r){
  const vals=[r?.q,r?.A,r?.B,r?.C,r?.D].map(v=>String(v||'').trim());
  let score=100;
  if(!vals[0])score-=35;
  for(const v of vals.slice(1))if(!v)score-=16;
  if(vals.slice(1).some(v=>/^\d{3}\s*[.)-:]?$/.test(v)))score-=20;
  if(vals.some(v=>/\b(?:10[1-9]|1[0-9]{2}|200)\s*[.)-:]?\s*\([ABCD]\)/i.test(v)))score-=25;
  if(r?.parseMode==='legacy')score-=10;
  if(r?.parseMode==='geometry-no-options')score-=35;
  if(r?.parseMode==='legacy-p6')score-=15;
  return Math.max(0,Math.min(100,score));
}

function validateQuestionRecord(q){
  const issues=[];
  const textFields=['CauHoi','DapAnA','DapAnB','DapAnC','DapAnD'];
  for(const f of textFields)if(!String(q?.[f]??'').trim())issues.push(`THIEU_${f}`);
  const ans=String(q?.DapAnDung||'').trim().toUpperCase();
  if(ans && !/^[ABCD]$/.test(ans))issues.push('ANSWER_INVALID');
  if(suspiciousQuestion(q))issues.push('CONTENT_SUSPICIOUS');
  if(q?.Part==='Part 6' && (!q.GroupID || !String(q.PassageText||q.CauHoi||'').trim()))issues.push('P6_GROUP_OR_PASSAGE_MISSING');
  return issues;
}

function validateBank(rows){
  const report={tests:[],total:rows.length,missingNumbers:[],duplicates:[],invalid:[],lowConfidence:[],partCounts:{}};
  const seen=new Map(), byTest=new Map();
  for(const q of rows){
    const test=String(q.ActualTest||'');
    if(!byTest.has(test))byTest.set(test,[]);
    byTest.get(test).push(q);
    const key=test+'|'+q.CauSo;
    if(seen.has(key))report.duplicates.push(key); else seen.set(key,q);
    const part=q.Part||inferPart(Number(q.CauSo));
    report.partCounts[part]=(report.partCounts[part]||0)+1;
    const issues=validateQuestionRecord(q);
    if(issues.length)report.invalid.push({test,q:q.CauSo,issues});
    if(Number(q.ParseConfidence||0)<85)report.lowConfidence.push({test,q:q.CauSo,confidence:Number(q.ParseConfidence||0)});
  }
  for(const [test,qs] of byTest){
    const nums=[...new Set(qs.map(q=>Number(q.CauSo)).filter(Number.isFinite))].sort((a,b)=>a-b);
    const expected=[];
    for(const [a,b] of [[101,140],[141,152],[153,200]]){
      const have=nums.filter(n=>n>=a&&n<=b);
      if(have.length){
        for(let n=a;n<=b;n++)if(have.includes(n)===false && n>=Math.min(...have) && n<=Math.max(...have))report.missingNumbers.push(`${test}|${n}`);
      }
    }
    const counts={};for(const q of qs)counts[q.Part]=(counts[q.Part]||0)+1;
    report.tests.push({test,count:qs.length,parts:counts,range:nums.length?`${nums[0]}–${nums[nums.length-1]}`:'—'});
  }
  report.ok=!report.duplicates.length&&!report.invalid.length&&!report.missingNumbers.length;
  return report;
}

function extractPart6HeadersForTest(test, qs){
  const headers=[];
  const files=new Set(qs.map(q=>q.Source).filter(Boolean));
  const pages=pageInfo.filter(p=>{
    if(p.actual===test)return true;
    return p.sourceFile && files.has(p.sourceFile);
  });
  for(const p of pages){
    const t=String(p.text||'')
      ...
      .replace(/\s+/g,' ')
      .trim();
    // Accept normal PDF extraction, OCR variants, and line-wrap differences:
    // "Questions 141 to 143 refer to the following..."
    const re=/Questions?\s*(141|142|143|144|145|146|147|148|149|150|151|152)\s*(?:-|to|through|thru)\s*(141|142|143|144|145|146|147|148|149|150|151|152)\s*(?:refer(?:s)?|refe?r|r[e3]fer)\s*(?:to)?/gi;
    let m;
    while((m=re.exec(t))){
      const a=Number(m[1]),b=Number(m[2]);
      if(b>=a)headers.push({start:a,end:b,page:p.page,sourceFile:p.sourceFile});
    }
  }
  const uniq=[];const seen=new Set();
  for(const h of headers){const k=`${h.start}-${h.end}`;if(!seen.has(k)){seen.add(k);uniq.push(h)}}
  return uniq.sort((a,b)=>a.start-b.start);
}

function repairPart6Groups(rows){
  const p6=rows.filter(r=>r.Part==='Part 6').sort((a,b)=>String(a.ActualTest).localeCompare(String(b.ActualTest))||a.CauSo-b.CauSo);
  const byTest=new Map();
  for(const r of p6){if(!byTest.has(r.ActualTest))byTest.set(r.ActualTest,[]);byTest.get(r.ActualTest).push(r);}
  let repaired=0;
  for(const [test,qs] of byTest){
    const explicit=extractPart6HeadersForTest(test,qs);
    let groupIndex=0;
    let used=new Set();
    if(explicit.length){
      // Source header is authoritative. Never let an earlier bad GroupID
      // override the actual "Questions X to Y refer to..." boundary.
      for(const h of explicit){
        const members=qs.filter(q=>q.CauSo>=h.start&&q.CauSo<=h.end);
        if(!members.length)continue;
        const groupId=`P6-T${String(test.match(/(\d+)$/)?.[1]||1).padStart(2,'0')}-G${String(++groupIndex).padStart(2,'0')}`;
        for(const q of members){
          used.add(q.CauSo);
          if(q.GroupID!==groupId){q.GroupID=groupId;repaired++;}
          q.GroupStart=h.start;q.GroupEnd=h.end;
          q.GroupIndex=groupIndex;
        }
      }
    }
    // Fallback only for questions not covered by an explicit source header.
    // Split contiguous runs into safe 2–4 question passages; never merge a gap.
    for(let i=0;i<qs.length;){
      if(used.has(qs[i].CauSo)){i++;continue;}
      const start=qs[i].CauSo;
      let j=i+1,last=start;
      while(j<qs.length && qs[j].CauSo===last+1 && !used.has(qs[j].CauSo) && (last-start+1)<4){last=qs[j].CauSo;j++;}
      const groupId=`P6-T${String(test.match(/(\d+)$/)?.[1]||1).padStart(2,'0')}-G${String(++groupIndex).padStart(2,'0')}`;
      const passage=qs.slice(i,j).map(q=>String(q.PassageText||q.CauHoi||'').trim()).find(Boolean)||'';
      for(const q of qs.slice(i,j)){
        if(q.GroupID!==groupId){q.GroupID=groupId;repaired++;}
        q.GroupStart=start;q.GroupEnd=last;q.GroupIndex=groupIndex;
        if(!q.PassageText)q.PassageText=passage;
      }
      i=j;
    }
  }
...
function makeRecord(r,part,group){
  ...
  const confidence=adaptiveConfidence(r);
  const rec={
    MaCau:`${part.replace(' ','')}-T${tid}-${num}`,Part:part,CauSo:num,ActualTest:t,
    GroupID:gid,ChuDe:'PDF người dùng chọn',
    DangBai:part==='Part 5'?'Hoàn thành câu':part==='Part 6'?'Đọc đoạn văn và chọn đáp án':'Đọc hiểu theo bài',
    CauHoi:r.q||'',PassageText:r.PassageText||'',BlankNumber:r.BlankNumber||num,
    DapAnA:r.A||'',DapAnB:r.B||'',DapAnC:r.C||'',DapAnD:r.D||'',DapAnDung:'',
    GiaiThich:'',PassageImageURL:'',HinhBaiDoc:'',
    SourcePageStart:r.page||'',SourcePageEnd:r.page||'',Source:src,
    DataStatus:'SOURCE_PDF_PARSED_PENDING_REVIEW',
    GroupIndex:group?Number(group.id.match(/G(\d+)$/)?.[1]||0):0,
    GroupStart:group?.start||'',GroupEnd:group?.end||'',
    ReviewStatus:'PENDING',
    QualityStatus:confidence>=85?'PASS':'REPAIR',
    ParseConfidence:confidence,
    ParseMode:r.parseMode||'unknown',
    ValidationIssues:[]
  };
  rec.ValidationIssues=validateQuestionRecord(rec);
  if(rec.ValidationIssues.length)rec.ReviewStatus='INCOMPLETE';
  return rec;
}

function renderSummary(){
  const tests=[...new Set(bank.map(x=>x.ActualTest))];
  const counts={};for(const q of bank)counts[q.Part]=(counts[q.Part]||0)+1;
  const report=validateBank(bank);
  window.__toeicSmartReport=report;
  $('summary').innerHTML=`<b>${bank.length}</b> câu · <b>${tests.length}</b> Actual Test · Part 5: <b>${counts['Part 5']||0}</b> · Part 6: <b>${counts['Part 6']||0}</b> · Part 7: <b>${counts['Part 7']||0}</b><br><span class="${report.ok?'ok':'bad'}">Smart Validator: ${report.ok?'PASS':'REPAIR REQUIRED'} · ${report.invalid.length} lỗi nội dung · ${report.duplicates.length} trùng · ${report.missingNumbers.length} số câu thiếu · ${report.lowConfidence.length} độ tin cậy thấp</span>`;
  $('tests').innerHTML=tests.map(t=>{
    const qs=bank.filter(q=>q.ActualTest===t),tr=report.tests.find(x=>x.test===t)||{};
    return `<div class="test-card"><h3>${esc(t)}</h3><span class="badge">${qs.length} câu</span><div class="small">${['Part 5','Part 6','Part 7'].filter(p=>qs.some(q=>q.Part===p)).map(p=>p+': '+qs.filter(q=>q.Part===p).length).join(' · ')}</div><div class="small">Range: ${esc(tr.range||'—')} · ${qs.filter(q=>Number(q.ParseConfidence||0)<85).length?'⚠️ cần repair':'✓ confidence ổn'}</div></div>`;
  }).join('');
}
...
async function scan(){
 ...
 try{
  ...
  // Smart Adaptive post-pass: repair group metadata, then validate before answer merge.
  const p6Repaired=repairPart6Groups(bank);
  for(const q of bank){
    q.ValidationIssues=validateQuestionRecord(q);
    if(q.ReviewStatus!=='VERIFIED') q.ReviewStatus=q.ValidationIssues.length?'INCOMPLETE':'PENDING';
    q.QualityStatus=q.ValidationIssues.length?'REPAIR':(Number(q.ParseConfidence||0)>=85?'PASS':'REPAIR');
  }
  const smartReport=validateBank(bank);
...
async function recognizeAnswers(){
 ...
 try{
  ...
  renderAnswers();renderSummary();renderBank();status('✅ Đã phân tích đề và ghép đáp án từ Answer Key (trong PDF đề hoặc PDF đáp án). Tất cả đáp án vẫn ở trạng thái chờ xác nhận.');
...
async function exportZip(official=false){
 ...
 const zip=new JSZip(),out=JSON.parse(JSON.stringify(bank)),stamp=official?'verified':'draft';zip.file(`TOEIC_BANK_BUILDER_${stamp}.json`,JSON.stringify(out,null,2));zip.file('README_V45.1.6_SMART_ADAPTIVE_PARSER.txt',`V45.1.6 TOEIC Smart Adaptive Parser + AI Repair\n\nPart 5 / Part 6 / Part 7.\nNguồn đề: ${sourceFiles.map(f=>f.name).join(' | ')||''}\nNguồn đáp án: ${$('answerFile')?.files[0]?.name||'không dùng'}\n${official?'Bản đã được người dùng rà soát/xác nhận.':'Bản nháp, cần rà soát.'}\n`);
 ...
 status('📦 Đang đóng ZIP…');const b=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`V45.1.6_TOEIC_BANK_${stamp}.zip`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),3000);status(`✅ Đã xuất ${official?'bản chính thức':'bản nháp'} ZIP.`)
}
...
window.openToeicBankBuilder=function(){const m=document.getElementById('toeic-bank-builder-modal');if(!m){console.error('[TOEIC Bank Builder] Modal not found');return false;}m.style.display='flex';document.body.style.overflow='hidden';try{addBuilderUI();bindBuilderEvents();}catch(e){console.error(e);status('❌ Không khởi tạo Builder: '+(e.message||e));}return true};
window.__toeicBankBuilderLoaded=true;
