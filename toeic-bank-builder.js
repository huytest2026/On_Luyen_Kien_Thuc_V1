...
/* V44.1.9 — Integrated TOEIC Bank Builder
...
async function pageLayout(doc,n){
 ...
 // Detect true two-column layouts by clustering the line start positions.
 // The previous largest-gap method was fragile because wrapped lines have many
 // different x positions; on real TOEIC PDFs it could merge the right column
 // (105...) into the left column (101...), causing A-D to disappear.
 let cols=1;
 if(lines.length>=8){
  let c1=Math.min(...lines.map(l=>l.x)),c2=Math.max(...lines.map(l=>l.x));
  for(let pass=0;pass<6;pass++){
   const g1=lines.filter(l=>Math.abs(l.x-c1)<=Math.abs(l.x-c2));
   const g2=lines.filter(l=>Math.abs(l.x-c1)>Math.abs(l.x-c2));
   if(!g1.length||!g2.length)break;
   c1=g1.reduce((s,l)=>s+l.x,0)/g1.length;
   c2=g2.reduce((s,l)=>s+l.x,0)/g2.length;
  }
  const sep=Math.abs(c2-c1),leftC=Math.min(c1,c2),rightC=Math.max(c1,c2);
  const left=lines.filter(l=>Math.abs(l.x-leftC)<=Math.abs(l.x-rightC));
  const right=lines.filter(l=>Math.abs(l.x-leftC)>Math.abs(l.x-rightC));
  if(sep>Math.max(100,vp.width*.18)&&left.length>=3&&right.length>=3){
   cols=2;
   lines.sort((a,b)=>{const ca=Math.abs(a.x-leftC)<=Math.abs(a.x-rightC)?0:1,cb=Math.abs(b.x-leftC)<=Math.abs(b.x-rightC)?0:1;return ca-cb||b.y-a.y||a.x-b.x});
  }
 }
...
function renderBank(){
 ...
 if(btnVerifyAll)btnVerifyAll.onclick=()=>{
   const eligible=bank.filter(q=>q.ReviewStatus==='PENDING'&&isReviewComplete(q));
   if(!eligible.length)return;
   if(!confirm(`Xác nhận tất cả ${eligible.length} câu PENDING đã đủ câu hỏi, A/B/C/D và đáp án?`))return;
   eligible.forEach(q=>{q.ReviewStatus='VERIFIED';q.DataStatus='SOURCE_VERIFIED_BY_USER';q.AnswerStatus='VERIFIED_BY_USER'});
   renderBank();
   if(editingIndex>=0)openEditor(editingIndex);
   status(`✅ Đã xác nhận hàng loạt ${eligible.length} câu PENDING.`);
 };
...
async function exportZip(official=false){
 ...
 const zip=new JSZip(),out=JSON.parse(JSON.stringify(bank)),stamp=official?'verified':'draft';zip.file(`TOEIC_BANK_BUILDER_${stamp}.json`,JSON.stringify(out,null,2));zip.file('README_V44.1.9_BUILDER.txt',`V44.1.9 Integrated TOEIC Bank Builder\n\nPart 5 / Part 6 / Part 7.\nNguồn đề: ${sourceFiles.map(f=>f.name).join(' | ')||''}\nNguồn đáp án: ${$('answerFile')?.files[0]?.name||'không dùng'}\n${official?'Bản đã được người dùng rà soát/xác nhận.':'Bản nháp, cần rà soát.'}\n`);
 ...
 status('📦 Đang đóng ZIP…');const b=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`V44.1.9_TOEIC_BANK_${stamp}.zip`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),3000);status(`✅ Đã xuất ${official?'bản chính thức':'bản nháp'} ZIP.`)
}
