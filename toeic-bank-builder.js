...
function renderBank(){
 ...
 // V45.0.7 — Part 6 grouped display.
// The parser keeps the complete passage in every Part 6 record so export/reuse
// remains backward compatible.  In the review table, however, a passage must
// be shown ONCE per GroupID; otherwise 141–143 (or 144–146, etc.) look like
// three copies of the same reading.  Each row still shows only its own A/B/C/D.
const p6PassageShown=new Set();
$('bankRows').innerHTML=bank.map((q,i)=>{
  const isP6=q.Part==='Part 6';
  const gid=q.GroupID||`${q.ActualTest}-P6-${q.CauSo}`;
  const showPassage=isP6 && !p6PassageShown.has(gid);
  if(isP6 && showPassage)p6PassageShown.add(gid);
  let qHtml='';
  if(isP6){
    const passage=q.PassageText||q.CauHoi||'';
    qHtml=(showPassage
      ? `<div class="small" style="margin-bottom:6px"><b>📖 PASSAGE ${esc(gid)}</b></div><div style="white-space:pre-wrap;margin-bottom:8px">${esc(passage)}</div>`
      : `<div class="small" style="margin-bottom:6px;color:#666">↳ Cùng passage ${esc(gid)}</div>`);
    qHtml+=`<b>Câu ${q.CauSo}</b>`;
  }else{
    qHtml=`<b>${esc(q.CauHoi)}</b>`;
  }
  qHtml+=`<br>A. ${esc(q.DapAnA)}<br>B. ${esc(q.DapAnB)}<br>C. ${esc(q.DapAnC)}<br>D. ${esc(q.DapAnD)}`;
  return `<tr data-i="${i}" style="cursor:pointer"><td>${esc(q.ActualTest)}</td><td>${esc(q.Part)}${q.GroupID?'<br><span class="small">'+esc(q.GroupID)+'</span>':''}</td><td><b>${q.CauSo}</b></td><td>${qHtml}</td><td><b>${esc(q.DapAnDung||'—')}</b></td><td class="${q.ReviewStatus==='VERIFIED'?'ok':'bad'}">${esc(q.ReviewStatus)}${q.QualityStatus&&q.QualityStatus!=='PASS'?'<br><span class="small">'+esc(q.QualityStatus)+'</span>':''}</td></tr>`;
}).join('');document.querySelectorAll('#bankRows tr').forEach(tr=>tr.onclick=()=>openEditor(Number(tr.dataset.i)));$('btnExportOfficial').disabled=!(bank.length&&bank.every(q=>q.ReviewStatus==='VERIFIED'&&/^[ABCD]$/.test(q.DapAnDung||'')))}
