...
function parseGeometryQuestion(region,p){
 ...
 const localN=(region.n>=101&&region.n<=140)?region.n-100:region.n;
 const body=lines.map(line=>({ ...line, text:clean(line.text.replace(new RegExp('^(?:'+region.n+'|'+localN+')[.)\\-:]?\\s*'),'').trim()) })).filter(line=>line.text);
...
function parseQuestionsFromPages(pages){
 ...
 for(const p of pages){
  ...
  if(geo.length){
   // Geometry remains the primary parser. If it lost one or more choices,
   // use the page-text parser only to fill those missing fields.
   const chunks=splitQuestionChunks(String(p.text||''));
   for(const r of geo){
    const ch=chunks.find(x=>x.n===r.n);
    if(ch){
     const po=parseOptions(ch.text);
     if(!r.q && po.q)r.q=po.q;
     if(!r.A && po.A)r.A=po.A;
     if(!r.B && po.B)r.B=po.B;
     if(!r.C && po.C)r.C=po.C;
     if(!r.D && po.D)r.D=po.D;
     if((po.A||po.B||po.C||po.D) && r.parseMode==='geometry')r.parseMode='geometry+text-fallback';
    }
    if(!rows.some(x=>x.n===r.n&&x.page===r.page&&x.sourceFile===r.sourceFile))rows.push(r)
   }
   continue;
  }
