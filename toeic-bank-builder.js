...
function buildColumnRegions(p){
 ...
 const allowLocal=/Actual\s*Test\s*04$/i.test(String(p.actual||''));
 const qRe=allowLocal?/^(?:(10[1-9]|1[0-9]{2}|200)|([1-9]|[1-3]\d|40))\s*[.)\-:]?(?:\s|$)/:/^(10[1-9]|1[0-9]{2}|200)\s*[.)\-:]?(?:\s|$)/;
 for(let ci=0;ci<cols.length;ci++){
   ...
   for(const line of lines){
     ...
     const n=m[1]?Number(m[1]):Number(m[2])+100;
...
function parseAnswerText(text,allowLocal=false){
 ...
 if(allowLocal){const lr=/(?:^|[\s|])((?:[1-9]|[1-3]\d|40))\s*[.)\-:]?\s*\(?([ABCD])\)?\b/gi;while((m=lr.exec(s)))out[Number(m[1])+100]=m[2].toUpperCase();}
 ...
 if(allowLocal){const lr2=/(?:^|[\s|])((?:[1-9]|[1-3]\d|40))\s*[.)\-:]?\s*\(?([08])\)?\b/gi;while((m=lr2.exec(s))){const n=Number(m[1])+100,a=m[2]==='8'?'B':'D';if(!out[n])out[n]=a}}
 return out;
}
 ...
 try{
  ...
  for(const [test,qs] of Object.entries(byTest)){
   ...
   for(const item of pagesToUse){
    ...
    const a=parseAnswerText(text,testNo===4);
