...
function renderBank(){
 ...
 if(btnPendingAll)btnPendingAll.onclick=()=>{
   ...
   if(!confirm(`Đưa tất cả ${bank.length} câu về trạng thái PENDING?`))return;
   bank.forEach(q=>q.ReviewStatus='PENDING');
   renderBank();
   if(editingIndex>=0)openEditor(editingIndex);
   status(`↩ Đã đưa tất cả ${bank.length} câu về PENDING.`);
 };
