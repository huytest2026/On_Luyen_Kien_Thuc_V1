...
function parsePart6GeometryQuestionsFromPage(p){
 ...
 const mid=Number(p.mid||p.width/2||300);
 // Part 6 passages are logically single-flow even when the PDF page has
 const cols=[items];
 const out=[];
 const qRe=/^(141|142|143|144|145|146|147|148|149|150|151|152)\s*[.)\-:]?/i;
 const optRe=/^\s*(?:\([ABCD]\)|[ABCD]\s*[.)\-:])\s*/i;
 const makeLines=arr=>{
   const ys=[];
   for(const it of arr){
     let row=ys.find(r=>Math.abs(r.y-it.y)<=3);
 ...
 const lineIsOption=txt=>optRe.test(txt)||/^(?:141|142|143|144|145|146|147|148|149|150|151|152)\s*[.)\-:]?\s*\([ABCD]\)/i.test(txt);
 for(let ci=0;ci<cols.length;ci++){
   const col=cols[ci].slice();
   const lines=makeLines(col);
   const anchors=[];
   for(const line of lines){
     ...
     if(m)anchors.push({n:Number(m[1]),y:line.y,x:line.x});
   }
   const uniq=[];const seen=new Set();
   for(const a of anchors){const k=a.n+'|'+Math.round(a.y);if(!seen.has(k)){seen.add(k);uniq.push(a)}}
   if(!uniq.length)continue;

   // Build a clean passage stream for this column. Question labels and option
   // rows are removed; every PDF "-------" blank becomes a searchable marker.
   const streamLines=[];
   for(const line of lines){
     if(lineIsOption(line.text))continue;
     let parts=[];
     for(const it of line.items){
       const tx=String(it.text||'').trim();
       if(!tx)continue;
       if(/^(?:141|142|143|144|145|146|147|148|149|150|151|152)\s*[.)\-:]?$/i.test(tx))continue;
       if(/^\(?[ABCD]\)?[.)\-:]?$/.test(tx))continue;
       if(/^-{3,},?$/.test(tx)) parts.push('§§BLANK§§');
       else parts.push(tx);
     }
     const text=clean(parts.join(' '));
     if(text)streamLines.push({y:line.y,text});
   }
   const stream=streamLines.map(x=>x.text).join(' ');
   const markers=[];
   // Locate each blank by the nearest question label below it. In the source
   // PDFs the label is normally 18pt below the blank, so use geometry first.
   for(const a of uniq){
     let best=null,bestScore=Infinity;
     for(const it of col){
       const tx=String(it.text||'').trim();
       if(!/^-{3,},?$/.test(tx))continue;
       const dy=it.y-a.y;
       if(dy<0||dy>38)continue;
       const dx=Math.abs((it.x||0)-((a.x||0)+26));
       const score=dy*2+dx*0.15;
       if(score<bestScore){bestScore=score;best=it;}
     }
     if(!best)continue;
     const placeholder='§§Q'+a.n+'§§';
     markers.push({n:a.n,placeholder,y:best.y,x:best.x});
   }
   if(!markers.length)continue;
   // Replace each blank in the flattened stream with its question marker.
   let stream2=stream;
   for(const m of markers)stream2=stream2.replace('§§BLANK§§',m.placeholder);
   markers.sort((a,b)=>b.y-a.y);
   for(const m of markers){
     const pos=stream2.indexOf(m.placeholder);if(pos<0)continue;
     let start=0,end=stream2.length;
     for(let i=pos-1;i>=0;i--){if(/[.!?]/.test(stream2[i])){start=i+1;break;}}
     for(let i=pos+m.placeholder.length;i<stream2.length;i++){if(/[.!?]/.test(stream2[i])){end=i+1;break;}}
     let q=stream2.slice(start,end).replace(m.placeholder,'-------').trim();
     q=clean(q);
     // Part 6 often starts on the same page as an e-mail/memo/letter header.
     // Strip that non-question header when sentence punctuation is absent before
     // the first real sentence.
     const dear=q.match(/\b(?:Dear|Hi|Hello)\s+[^,]{2,80},\s*/i);
     if(dear&&dear.index<180)q=q.slice(dear.index+dear[0].length).trim();
     else if(/^From:/i.test(q)){
       const greet=q.match(/\b[A-Z][A-Za-z.'-]{1,40},\s+(?=[A-Z][a-z])/);
       if(greet&&greet.index<180)q=q.slice(greet.index+greet[0].length).trim();
     }
     // Avoid accidentally returning an entire heading/passage when the PDF has
     // no sentence punctuation around the blank.
     if(q.length>450){
       const left=stream2.slice(Math.max(0,start),pos).split(/\s+/).slice(-45).join(' ');
       const right=stream2.slice(pos+m.placeholder.length,end).split(/\s+/).slice(0,45).join(' ');
       q=clean([left,'-------',right].join(' '));
     }
     const region=buildColumnRegions({...p,items:col,mid:p.columns===2?(ci?mid:mid):mid}).find(r=>r.n===m.n&&Math.abs(r.y-(anchors.find(a=>a.n===m.n)?.y||r.y))<4);
     if(!region)continue;
     const parsed=parseGeometryQuestion(region,p);
     // parseGeometryQuestion may contain the passage after the options; keep only
     // its options, while the sentence-centric stem comes from the blank itself.
     out.push({...parsed,q,A:parsed.A,B:parsed.B,C:parsed.C,D:parsed.D,parseMode:'geometry-p6-sentence'});
   }
 }
 return out;
}
