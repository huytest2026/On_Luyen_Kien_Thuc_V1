...
/* V44.1.6 — Integrated TOEIC Bank Builder
...
async function pageLayout(doc,n){
 ...
 // IMPORTANT: For TOEIC PDFs in 2-column layout, pdf.js can return items
 // from the left and right columns on the SAME Y coordinate.  Grouping by Y
 // first (the old code) therefore produced errors such as:
 // "101 ... that 105." and "B. 102.".
 // Split the raw text items into columns FIRST, then build lines inside each
 // column. This preserves the real reading order of the PDF.
 ...
 let cut=null,columns=1;
 if(gap>Math.max(70,vp.width*.12)&&split>0&&split<xs.length){
   const candidate=(xs[split-1]+xs[split])/2;
   const leftCount=items.filter(i=>i.x<=candidate).length;
   const rightCount=items.filter(i=>i.x>candidate).length;
   if(leftCount>=3&&rightCount>=3){cut=candidate;columns=2;}
 }
 ...
 let lines;
 if(cut!==null){
   const left=makeLines(items.filter(i=>i.x<=cut));
   const right=makeLines(items.filter(i=>i.x>cut));
   lines=[...left,...right];
 }else{
   lines=makeLines(items);
 }

 return {page:n,text:lines.map(l=>l.text).join('\n'),lines,width:vp.width,height:vp.height,columns};
}
...
function parseOptions(chunk){
 // Do not mistake a question number interleaved by a 2-column PDF extractor
 // for an answer choice, e.g. "B. 102.".
 const s=clean(chunk).replace(/\s+/g,' ');
 const re=/(?:^|\s)(\(?([ABCD])\)?)[.)\-:]?\s+(?!10[1-9](?:[.)\-:]|\s)|1[1-9]\d(?:[.)\-:]|\s)|19\d(?:[.)\-:]|\s)|200(?:[.)\-:]|\s))/gi;
 const hits=[];let m;
 while((m=re.exec(s)))hits.push({letter:m[2].toUpperCase(),start:m.index+m[0].length,markerStart:m.index});
 const o={A:'',B:'',C:'',D:''};
 if(!hits.length)return {q:s,...o};
 const q=s.slice(0,hits[0].markerStart).trim();
 for(let i=0;i<hits.length;i++){
   const end=i+1<hits.length?hits[i+1].start:s.length;
   const key=hits[i].letter;
   const val=clean(s.slice(hits[i].start,end)).replace(/^[.)\-:]\s*/,'').trim();
   if(!/^((?:10[1-9]|1[1-9]\d|19\d|200))[.)\-:]?$/.test(val)) o[key]=val;
 }
 return {q:clean(q),...o};
}
function parseQuestionsFromPages(pages){
 ...
 for(const p of pages){
  ...
  for(const ch of splitQuestionChunks(p.text)){
   ...
   // Remove only a trailing question-number marker left by 2-column extraction.
   const qq=clean(po.q).replace(/\s+(?:10[1-9]|1[1-9]\d|19\d|200)[.)\-:]\s*$/,'').trim();
   if(!rows.some(r=>r.n===ch.n&&r.page===p.page))
     rows.push({n:ch.n,q:qq,A:po.A,B:po.B,C:po.C,D:po.D,page:p.page,actual:p.actual,sourceFile:p.sourceFile,doc:p.doc});
  }
 }
 return rows;
}
...
function isReviewComplete(q){
 ...
 return !!(q&&has(q.CauHoi)&&has(q.DapAnA)&&has(q.DapAnB)&&has(q.DapAnC)&&has(q.DapAnD)&&/^[ABCD]$/.test(String(q.DapAnDung||'').trim().toUpperCase()));
}
function syncIncompleteStatuses(){
 for(const q of bank){
   if(!isReviewComplete(q)){
...
function renderBank(){
 ...
 const missing=bank.filter(q=>!q.CauHoi||![q.DapAnA,q.DapAnB,q.DapAnC,q.DapAnD].every(Boolean)||!/^[ABCD]$/.test(q.DapAnDung||'')).length,verified=bank.filter(q=>q.ReviewStatus==='VERIFIED').length;
 $('issues').className=missing?'dangerbox':'warnbox';
 $('issues').innerHTML=`<span><b>${bank.length}</b> câu · <b>${verified}</b> đã xác nhận · <b>${bank.length-verified}</b> cần kiểm tra · <b>${missing}</b> câu thiếu nội dung/lựa chọn.</span>
...
async function exportZip(official=false){
 ...
 const zip=new JSZip(),out=JSON.parse(JSON.stringify(bank)),stamp=official?'verified':'draft';zip.file(`TOEIC_BANK_BUILDER_${stamp}.json`,JSON.stringify(out,null,2));zip.file('README_V44.1.6_BUILDER.txt',`V44.1.6 Integrated TOEIC Bank Builder\n\nPart 5 / Part 6 / Part 7.\nNguồn đề: ${sourceFiles.map(f=>f.name).join(' | ')||''}\nNguồn đáp án: ${$('answerFile')?.files[0]?.name||'không dùng'}\n${official?'Bản đã được người dùng rà soát/xác nhận.':'Bản nháp, cần rà soát.'}\n`);
