...
function parseGeometryQuestion(region,p){
 ...
 const body=lines.map(line=>({ ...line, text:clean(line.text.replace(new RegExp('^(?:'+region.n+'|'+(region.n-100)+')[.)\-:]?\s*'),'').trim()) })).filter(line=>line.text);
...
function parseAnswerText(text){
 ...
 const re=/(?:^|[\s|])((?:10[1-9]|1[1-9]\d|19\d|200)|(?:[1-9]|[1-3]\d|40))\s*[.)\-:]?\s*\(?([ABCD])\)?\b/gi;let m;while((m=re.exec(s))){let n=Number(m[1]);if(n>=1&&n<=40)n+=100;out[n]=m[2].toUpperCase();}
 // OCR sometimes turns B into 8 or D into 0. Only use when immediately following the question number.
 const re2=/(?:^|[\s|])((?:10[1-9]|1[1-9]\d|19\d|200)|(?:[1-9]|[1-3]\d|40))\s*[.)\-:]?\s*\(?([08])\)?\b/gi;while((m=re2.exec(s))){let n=Number(m[1]);if(n>=1&&n<=40)n+=100;const a=m[2]==='8'?'B':'D';if(!out[n])out[n]=a}
 return out;
}
