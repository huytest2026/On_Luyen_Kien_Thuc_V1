...
async function pageLayout(doc,n){
 ...
 // IMPORTANT: group PDF text into columns BEFORE grouping by Y.
 // A two-column TOEIC page can contain question 101 on the left and 105 on
 // the right at nearly the same Y. Grouping by Y first interleaves the columns
 // and makes the option lines disappear from the wrong question.
 const mid=vp.width/2;
 const left=items.filter(i=>i.x+i.w/2 < mid);
 const right=items.filter(i=>i.x+i.w/2 >= mid);
 const leftMin=left.length?Math.min(...left.map(i=>i.x)):Infinity;
 const rightMin=right.length?Math.min(...right.map(i=>i.x)):Infinity;
 const rightMax=right.length?Math.max(...right.map(i=>i.x)):0;
 const leftMax=left.length?Math.max(...left.map(i=>i.x+i.w)):0;
 const twoCols=left.length>=4&&right.length>=4&&rightMin-leftMax>Math.max(25,vp.width*.025);
 const columnSets=twoCols?[left,right]:[items];
 const lines=[];
 for(let ci=0;ci<columnSets.length;ci++){
  const arr=columnSets[ci],ys=[];
  for(const it of arr){
  ...
  for(const r of ys){
   ...
   lines.push({column:ci,x:Math.min(...r.items.map(i=>i.x)),y:r.y,text:clean(r.items.map(i=>i.text).join(' '))});
  }
 }
 // Preserve natural reading order: left column top-to-bottom, then right.
 lines.sort((a,b)=>a.column-b.column||b.y-a.y);
 return {page:n,text:lines.map(l=>l.text).join('\n'),lines,width:vp.width,height:vp.height,columns:twoCols?2:1};
}
...
function renderBank(){
 ...
 <button id="btnVerifyAll" class="btn primary" style="margin-left:12px;padding:6px 10px" ${bank.some(q=>q.ReviewStatus==='PENDING'&&isReviewComplete(q))?'':'disabled'}>✅ Xác nhận tất cả PENDING</button>`;
 ...
 if(btnVerifyAll)btnVerifyAll.onclick=()=>{
   ...
   if(!confirm(`Xác nhận ${candidates.length} câu PENDING đã đủ Câu hỏi + A/B/C/D + đáp án?`))return;
   candidates.forEach(q=>{q.ReviewStatus='VERIFIED';q.DataStatus='SOURCE_VERIFIED_BY_USER';q.AnswerStatus='VERIFIED_BY_USER'});
   renderBank();
   if(editingIndex>=0)openEditor(editingIndex);
   status(`✅ Đã xác nhận ${candidates.length} câu PENDING. Câu INCOMPLETE vẫn giữ nguyên.`);
 };
