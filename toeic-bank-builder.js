...
async function pageLayout(doc,n){
 ...
 if(leftQ.length>=1&&rightQ.length>=1){
   const lx=leftQ.reduce((a,b)=>a+b.x,0)/leftQ.length;
   const rx=rightQ.reduce((a,b)=>a+b.x,0)/rightQ.length;
   // A real TOEIC two-column page can have a very narrow gutter; do NOT
   // require a 25% page-width gap between the two question-number anchors.
   // The anchors themselves are the strongest evidence of two columns.
   if(rx>lx+vp.width*.20){twoCol=true;cut=(lx+rx)/2;}
 }
 ...
 const columnTexts=twoCol?[(left||[]).map(l=>l.text).join('\n'),(right||[]).map(l=>l.text).join('\n')]:[lines.map(l=>l.text).join('\n')];
 return {page:n,text:lines.map(l=>l.text).join('\n'),lines,width:vp.width,height:vp.height,columns:twoCol?2:1,columnTexts};
}
...
function parseQuestionsFromPages(pages){
 const rows=[];for(const p of pages){
  ...
  // CRITICAL: parse each physical PDF column independently. Never flatten
  // the left and right columns into one stream before detecting question numbers.
  const streams=Array.isArray(p.columnTexts)&&p.columnTexts.length>1?p.columnTexts:[p.text||''];
  for(const stream of streams){
   for(const ch of splitQuestionChunks(stream)){
