...
/* V45.0.7 — Integrated TOEIC Bank Builder
...
function parsePart6GeometryQuestionsFromPage(p){
 ...
 // Remove isolated question numbers left by PDF extraction, but DO NOT remove
 // the text of the passage around them.
...
function ensureP6Styles(){
 if(document.getElementById('p6-v4508-style'))return;
 const st=document.createElement('style');st.id='p6-v4508-style';st.textContent=`
 .p6-group{border:1px solid #cfd8e3;border-radius:10px;padding:10px;background:#fafcff}
 .p6-group-head{display:flex;justify-content:space-between;gap:12px;padding:6px 8px;margin-bottom:8px;background:#eef4f9;border-radius:7px}
 .p6-passage{white-space:pre-wrap;line-height:1.45;padding:10px;background:#fff;border:1px solid #e1e6eb;border-radius:8px;margin-bottom:10px}
 .p6-questions{display:grid;gap:8px}
 .p6-q{padding:9px 10px;border:1px solid #dfe5eb;border-radius:8px;background:#fff;cursor:pointer}
 .p6-q:hover{box-shadow:0 1px 5px rgba(0,0,0,.08)}
 .p6-q-head{display:flex;justify-content:space-between;gap:10px;margin-bottom:4px}
 .p6-stem{font-weight:600;margin:4px 0 6px}
 .p6-options{display:grid;grid-template-columns:1fr 1fr;gap:2px 18px;margin:4px 0 5px}
 .p6-group-row>td{padding:8px!important;background:#f7faff}
...
function renderBank(){
 ...
 // V45.0.8 — Part 6 TRUE GROUP REVIEW UI.
// Do not render Part 6 as 12 independent table rows. A Part 6 passage is one
// logical reading group (usually 3 blanks), followed by its questions. The
// parser/export data remains one record per question for backward compatibility,
// but the review UI renders one group card: PASSAGE once + all questions below.
function part6QuestionSentence(q){
  const text=String(q.PassageText||q.CauHoi||'').replace(/\r/g,'');
  const blanks=[...text.matchAll(/_{3,}/g)];
  const idx=Math.max(0,Number(q.CauSo)-141);
  const m=blanks[idx];
  if(!m)return '';
  const pos=m.index, left=text.slice(0,pos), right=text.slice(pos+m[0].length);
  const start=Math.max(left.lastIndexOf('\n'),left.lastIndexOf('. '),left.lastIndexOf('? '),left.lastIndexOf('! '))+1;
  let end=right.search(/[.!?](?:\s|$)/);
  if(end<0)end=Math.min(right.length,220);else end+=1;
  return clean((left.slice(start)+ ' ____' + right.slice(0,end)).replace(/\n+/g,' '));
}
function p6OptionHtml(q,i){
  const opts=[['A',q.DapAnA],['B',q.DapAnB],['C',q.DapAnC],['D',q.DapAnD]];
  return `<div class="p6-options">${opts.map(([l,v])=>`<div><b>${l}.</b> ${esc(v||'')}</div>`).join('')}</div>`;
}
function renderPart6Groups(){
  const groups6=[]; const indexByKey=new Map();
  bank.forEach((q,i)=>{
    if(q.Part!=='Part 6')return;
    const key=q.GroupID||`${q.ActualTest}-P6-${Math.floor((q.CauSo-141)/3)+1}`;
    if(!indexByKey.has(key)){indexByKey.set(key,groups6.length);groups6.push({key,test:q.ActualTest,qs:[]})}
    groups6[indexByKey.get(key)].qs.push({q,i});
  });
  return groups6.map(g=>{
    g.qs.sort((a,b)=>a.q.CauSo-b.q.CauSo);
    const first=g.qs[0].q;
    const passage=first.PassageText||first.CauHoi||'';
    const nums=g.qs.map(x=>x.q.CauSo).join(', ');
    const questionHtml=g.qs.map(({q,i})=>{
      const stem=part6QuestionSentence(q);
      const complete=isReviewComplete(q);
      return `<div class="p6-q" data-i="${i}" title="Nhấp để chỉnh sửa câu ${q.CauSo}">
        <div class="p6-q-head"><b>Câu ${q.CauSo}</b><span class="${q.ReviewStatus==='VERIFIED'?'ok':'bad'}">${esc(q.ReviewStatus)}</span></div>
        <div class="p6-stem">${esc(stem||`Chỗ trống ${q.CauSo-140} trong passage`)}</div>
        ${p6OptionHtml(q,i)}
        <div class="small">Đáp án: <b>${esc(q.DapAnDung||'—')}</b>${complete?'':' · ⚠ Thiếu dữ liệu'}</div>
      </div>`;
    }).join('');
    return `<tr class="p6-group-row"><td colspan="6">
      <div class="p6-group">
        <div class="p6-group-head"><b>📖 ${esc(g.test)} · ${esc(g.key)}</b><span>Câu ${nums}</span></div>
        <div class="p6-passage"><div class="small"><b>PASSAGE — hiển thị 1 lần</b></div><div>${esc(passage)}</div></div>
        <div class="p6-questions">${questionHtml}</div>
      </div>
    </td></tr>`;
  }).join('');
}
// Build rows in original bank order, replacing each contiguous Part 6 group by
// a single group row while leaving Part 5/7 rows exactly as before.
const outRows=[]; const doneP6=new Set();
bank.forEach((q,i)=>{
  if(q.Part==='Part 6'){
    const key=q.GroupID||`${q.ActualTest}-P6-${Math.floor((q.CauSo-141)/3)+1}`;
    if(!doneP6.has(key)){doneP6.add(key);outRows.push({type:'p6',key})}
    return;
  }
  let qHtml=`<b>${esc(q.CauHoi)}</b><br>A. ${esc(q.DapAnA)}<br>B. ${esc(q.DapAnB)}<br>C. ${esc(q.DapAnC)}<br>D. ${esc(q.DapAnD)}`;
  outRows.push({type:'normal',i,q,qHtml});
});
const p6Rows=renderPart6Groups();
// Insert grouped Part 6 cards at their natural position; normal rows keep the
// existing one-question layout.
const p6Map=new Map(); [...p6Rows.matchAll(/<tr class=\"p6-group-row\">[\s\S]*?<b>📖 ([^<]+) · ([^<]+)<\/b>/g)].forEach(()=>{});
const groupHtml=new Map();
for(const q of bank.filter(x=>x.Part==='Part 6')){const key=q.GroupID||`${q.ActualTest}-P6-${Math.floor((q.CauSo-141)/3)+1}`;if(!groupHtml.has(key)){const gq=bank.filter(x=>x.Part==='Part 6'&&(x.GroupID||`${x.ActualTest}-P6-${Math.floor((x.CauSo-141)/3)+1}`)===key);const first=gq[0], nums=gq.map(x=>x.CauSo).join(', '),passage=first.PassageText||first.CauHoi||'';const qs=gq.sort((a,b)=>a.CauSo-b.CauSo).map(x=>{const i=bank.indexOf(x),stem=part6QuestionSentence(x);return `<div class="p6-q" data-i="${i}"><div class="p6-q-head"><b>Câu ${x.CauSo}</b><span class="${x.ReviewStatus==='VERIFIED'?'ok':'bad'}">${esc(x.ReviewStatus)}</span></div><div class="p6-stem">${esc(stem||`Chỗ trống ${x.CauSo-140} trong passage`)}</div>${p6OptionHtml(x,i)}<div class="small">Đáp án: <b>${esc(x.DapAnDung||'—')}</b></div></div>`}).join('');groupHtml.set(key,`<tr class="p6-group-row"><td colspan="6"><div class="p6-group"><div class="p6-group-head"><b>📖 ${esc(first.ActualTest)} · ${esc(key)}</b><span>Câu ${nums}</span></div><div class="p6-passage"><div class="small"><b>PASSAGE</b> · hiển thị 1 lần</div><div>${esc(passage)}</div></div><div class="p6-questions">${qs}</div></div></td></tr>`);}}
let html=''; const emitted=new Set();
for(const row of outRows){
  if(row.type==='p6'){if(!emitted.has(row.key)){emitted.add(row.key);html+=groupHtml.get(row.key)||'';}continue;}
  const q=row.q,i=row.i;
  html+=`<tr data-i="${i}" style="cursor:pointer"><td>${esc(q.ActualTest)}</td><td>${esc(q.Part)}${q.GroupID?'<br><span class="small">'+esc(q.GroupID)+'</span>':''}</td><td><b>${q.CauSo}</b></td><td>${row.qHtml}</td><td><b>${esc(q.DapAnDung||'—')}</b></td><td class="${q.ReviewStatus==='VERIFIED'?'ok':'bad'}">${esc(q.ReviewStatus)}${q.QualityStatus&&q.QualityStatus!=='PASS'?'<br><span class="small">'+esc(q.QualityStatus)+'</span>':''}</td></tr>`;
}
$('bankRows').innerHTML=html;
document.querySelectorAll('#bankRows tr[data-i]').forEach(tr=>tr.onclick=()=>openEditor(Number(tr.dataset.i)));
document.querySelectorAll('#bankRows .p6-q[data-i]').forEach(el=>el.onclick=e=>{e.stopPropagation();openEditor(Number(el.dataset.i))});
...
async function exportZip(official=false){
 ...
 status('📦 Đang đóng ZIP…');const b=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`V45.0.7_TOEIC_BANK_${stamp}.zip`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),3000);status(`✅ Đã xuất ${official?'bản chính thức':'bản nháp'} ZIP.`)
}
