...
function buildColumnRegions(p){
 ...
 for(let ci=0;ci<cols.length;ci++){
   const col=cols[ci].slice().sort((a,b)=>b.y-a.y);
   const anchors=[];
   for(const it of col){const n=questionNumberFromItem(it.text);if(n>=101&&n<=200){
     // A real question number sits near the left edge of its column. Ignore
     // numeric tokens in body text, page headers and footers.
     const minX=Math.min(...col.map(x=>x.x));
     if(it.x<=minX+28)anchors.push({n,y:it.y,x:it.x});
   }}
   const uniq=[];const seen=new Set();for(const a of anchors){const k=a.n+'|'+Math.round(a.y);if(!seen.has(k)){seen.add(k);uniq.push(a)}}
   for(let i=0;i<uniq.length;i++){
     const a=uniq[i],next=uniq[i+1];
     const yTop=a.y+8,yBottom=next?next.y+8:-Infinity;
...
function suspiciousQuestion(q){
 ...
 if(vals.some(v=>/\b(?:10[1-9]|1[0-9]{2}|200)[.)-]\s*$/.test(v)))return true;
 return false;
}
