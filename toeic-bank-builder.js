...
const clean=s=>norm(s).replace(/www[.,]?nhantriviet\.com/gi,'').replace(/Go on\s*(to|the)?\s*next page/gi,'').replace(/TOEIC BOOK STORE/gi,'').replace(/Copyright\s+www\.Hackers\.co\.kr\s+All\s+rights\s+reserved\.?/gi,'').replace(/www\.Hackers\.co\.kr\s+All\s+rights\s+reserved\.?/gi,'').trim();
...
async function pageLayout(doc,n){
 ...
 // V44.1.15: separate columns BEFORE grouping by Y. The old parser grouped all
 // text at the same Y first, so 101/105 and their A-D choices could be merged.
 const xs=[...new Set(items.map(i=>Math.round(i.x*10)/10))].sort((a,b)=>a-b);let cut=null,bestGap=0;
 for(let i=1;i<xs.length;i++){const g=xs[i]-xs[i-1];if(g>bestGap){bestGap=g;cut=(xs[i-1]+xs[i])/2;}}
 const isTwoCol=!!(cut!=null&&bestGap>=Math.max(90,vp.width*0.16));
 const columnItems=isTwoCol?[items.filter(i=>i.x<=cut),items.filter(i=>i.x>cut)]:[items];const lines=[];
 for(const colItems of columnItems){const ys=[];for(const it of colItems){let row=ys.find(r=>Math.abs(r.y-it.y)<=3);if(!row){row={y:it.y,items:[]};ys.push(row);}row.items.push(it);}for(const r of ys.sort((a,b)=>b.y-a.y)){r.items.sort((a,b)=>a.x-b.x);const text=clean(r.items.map(i=>i.text).join(' '));if(text)lines.push({x:Math.min(...r.items.map(i=>i.x)),y:r.y,text});}}
 return {page:n,text:lines.map(l=>l.text).join('\n'),lines,width:vp.width,height:vp.height,columns:isTwoCol?2:1};
}
...
function parseOptions(chunk){
 ...
 const q=clean(lines.slice(0,hits[0].i).join(' '));
