...
function ensureP6Styles(){
 if(document.getElementById('p6-v4509-style'))return;
 const st=document.createElement('style');st.id='p6-v4509-style';st.textContent=`
 ...
 .p6-inline-q{margin:7px 0 12px;padding:0 0 4px 0;cursor:pointer}
 .p6-inline-stem{white-space:pre-wrap}
...
// Render Part 6 exactly as a TOEIC passage: one continuous passage, with the
// four choices inserted immediately after the sentence containing each blank.
// This mirrors the source PDF instead of showing a passage followed by a
// separate list of independent question cards.
function renderP6InlinePassage(g){
 const first=g.qs[0].q;
 let text=String(first.PassageText||first.CauHoi||'').replace(/\r/g,'').trim();
 // Normalize only the artificial blank marker; do not rewrite the source text.
 text=text.replace(/[-_]{3,}/g,'____');
 const qs=g.qs.slice().sort((a,b)=>a.q.CauSo-b.q.CauSo);
 let cursor=0, html='', blankIndex=0;
 const blankRe=/_{4,}/g;
 let m;
 while((m=blankRe.exec(text))){
   const qx=qs[blankIndex];
   if(!qx){break;}
   const i=qx.i, q=qx.q;
   // Include text through the blank, then continue until the sentence ends.
   ...
   html+=`<span class="p6-inline-q" data-i="${i}" title="Nhấp để chỉnh sửa câu ${q.CauSo}"><span class="p6-q-number">${q.CauSo}.</span>${esc(text.slice(m.index,Math.min(end,text.length)))}`;
   html+=p6OptionsBlock(q,i)+`</span>`;
   cursor=Math.min(end,text.length);
   blankIndex++;
   if(cursor>=text.length)break;
   // The next loop starts at the text immediately after this sentence.
   blankRe.lastIndex=cursor;
 }
 if(cursor<text.length)html+=`<span class="p6-segment">${esc(text.slice(cursor))}</span>`;
 // If extraction did not preserve blank markers, fall back to the complete passage
 // rather than silently losing source text.
 if(blankIndex!==qs.length){
   html=esc(text);
   for(let k=qs.length-1;k>=0;k--){
     const q=qs[k].q,i=q?g.qs[k].i:-1;
     const token=`____`;
...
function renderPart6Groups(){
 ...
 return groups6.map(g=>{
   ...
   const body=renderP6InlinePassage(g);
   return `<tr class="p6-group-row"><td colspan="6">
     <div class="p6-group">
       <div class="p6-group-head"><b>📖 ${esc(g.test)} · ${esc(g.key)}</b><span>Câu ${nums}</span></div>
       <div class="p6-passage"><div class="p6-meta"><b>PASSAGE</b> · ${esc(nums)} · trình bày theo đúng luồng đoạn đọc</div>${body}</div>
     </div>
   </td></tr>`;
 }).join('');
}
// Build rows in original bank order, replacing each Part 6 group by one
// passage row. Part 5/7 remain exactly one question per row.
...
for(const q of bank.filter(x=>x.Part==='Part 6')){
 const key=q.GroupID||`${q.ActualTest}-P6-${Math.floor((q.CauSo-141)/3)+1}`;
 if(groupHtml.has(key))continue;
 const gq=bank.filter(x=>x.Part==='Part 6'&&(x.GroupID||`${x.ActualTest}-P6-${Math.floor((x.CauSo-141)/3)+1}`)===key)
   .sort((a,b)=>a.CauSo-b.CauSo).map(x=>({q:x,i:bank.indexOf(x)}));
 groupHtml.set(key,renderPart6GroupsForOne(key,q.ActualTest,gq));
}
function renderPart6GroupsForOne(key,test,gq){
 const nums=gq.map(x=>x.q.CauSo).join(', ');
 return `<tr class="p6-group-row"><td colspan="6"><div class="p6-group"><div class="p6-group-head"><b>📖 ${esc(test)} · ${esc(key)}</b><span>Câu ${nums}</span></div><div class="p6-passage"><div class="p6-meta"><b>PASSAGE</b> · ${esc(nums)} · trình bày theo đúng luồng đoạn đọc</div>${renderP6InlinePassage({qs:gq})}</div></div></td></tr>`;
}
let html=''; const emitted=new Set();
for(const row of outRows){
 if(row.type==='p6'){
   if(!emitted.has(row.key)){emitted.add(row.key);html+=groupHtml.get(row.key)||'';}
   continue;
 }
...
document.querySelectorAll('#bankRows .p6-inline-q[data-i]').forEach(el=>el.onclick=e=>{e.stopPropagation();openEditor(Number(el.dataset.i))});
