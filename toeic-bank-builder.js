...
function parseQuestionsFromPages(pages){
 const rows=[];for(const p of pages){
  ...
  const hasP6=/\b(?:141|142|143|144|145|146|147|148|149|150|151|152)\s*[.)\-:]?\s*\(A\)/i.test(String(p.text||''));
  const parsed=hasP6?parsePart6QuestionsFromPage(p):[];
  if(parsed.length){for(const r of parsed){if(!rows.some(x=>x.n===r.n&&x.page===r.page))rows.push(r)};continue;}
