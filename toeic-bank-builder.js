...
function addBuilderUI(){
 ...
 const first=modal.querySelector('#pdfFile');if(first&&!modal.querySelector('#answerFile'))first.parentElement.insertAdjacentHTML('afterend','<div class="field" style="flex:2"><label>File PDF đáp án <span class="small">(tùy chọn, nhưng nên chọn)</span></label><input id="answerFile" type="file" accept="application/pdf"></div>');
 ...
 const p=modal.querySelector('.builder-content .card .small');if(p)p.innerHTML='Builder đọc PDF đề, tự nhận diện câu 101–200 và chia Part 5/6/7. Có thể chọn <b>PDF đáp án riêng</b>; đáp án được gán tự động nhưng luôn phải rà soát trước khi xuất chính thức.';
 const rule=modal.querySelector('.builder-content .card:last-of-type .small');if(rule)rule.innerHTML='<p>• Part 5: câu 101–140.</p><p>• Part 6: câu 141–152, tự nhóm 3 câu.</p><p>• Part 7: câu 153–200, nhóm theo chuỗi câu liên tiếp.</p><p>• PDF đáp án có thể ở file riêng; chấp nhận dạng 101 (B), 101 B, 101. B.</p><p>• Dữ liệu luôn ở trạng thái PENDING cho tới khi người dùng xác nhận.</p><p>• ZIP xuất ra dùng <b>TOEIC_PART7_GROUPS/</b>, không còn TOEIC_PART7_PASSAGES/.</p>';
}
addBuilderUI();
