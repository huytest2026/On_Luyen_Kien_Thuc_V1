...
async function pageLayout(doc,n){
 ...
 // TOEIC source PDFs used by this Builder have a real two-column page layout.
 // Do NOT detect the columns by the largest X gap: the text starts of words
 // inside a column can create many small gaps, while the gutter itself can be
 // only ~15-25pt. The reliable separator is the page center.
 const mid=vp.width/2;
 const leftItems=items.filter(i=>i.x < mid-8);
 const rightItems=items.filter(i=>i.x >= mid+8);
 const hasTwoColumns=leftItems.length>=8 && rightItems.length>=8;
 ...
 let lines,columns=1;
 if(hasTwoColumns){
   const left=makeLines(leftItems);
   const right=makeLines(rightItems);
   lines=[...left,...right];
   columns=2;
 }else{
