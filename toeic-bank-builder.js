/* V45.1.6 — Smart Adaptive Parser + Validator + Auto-Repair pipeline. */
...
function repairPart6Groups(rows){
  ...
  for(const [test,qs] of byTest){
    ...
    for(let i=0;i<qs.length;){
      ...
      while(j<qs.length && qs[j].CauSo===last+1){last=qs[j].CauSo;j++;}
      // TOEIC Part 6 passages are normally 2–4 questions. Never merge a gap.
      const existing=qs.slice(i,j).map(q=>q.GroupID).filter(Boolean);
      const groupId=(existing.length && new Set(existing).size===1)
        ? existing[0]
        : `P6-T${String(test.match(/(\d+)$/)?.[1]||1).padStart(2,'0')}-G${String(++groupIndex).padStart(2,'0')}`;
      ...
      for(const q of qs.slice(i,j)){
        ...
        if(!q.GroupStart)q.GroupStart=start;
        if(!q.GroupEnd)q.GroupEnd=last;
